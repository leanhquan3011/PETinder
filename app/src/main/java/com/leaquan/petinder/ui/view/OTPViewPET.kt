...
import com.leaquan.petinder.databinding.LayoutOtpBinding

class OTPViewPET @JvmOverloads constructor(
    context: Context,
    ...
    private var binding : LayoutOtpBinding

    // region values
    private val itemCount:Int /** number of view */
    private val showCursor:Boolean /** cursor is ..| */
    ...
    init {
        binding = LayoutOtpBinding.inflate(LayoutInflater.from(context), this, true)
        ...
        context.theme.obtainStyledAttributes(
            ...
            .apply {
                try {
                    itemCount = getInteger(R.styleable.OTPView_otp_itemCount, 1) //default 1 item
                    ...
                    errorBackgroundImage = getDrawable(R.styleable.OTPView_otp_errorBackgroundImage) ?: backgroundImage
    ...
    private fun changeFocus(increment: Boolean){
        ...
        when {
            ...
            focusIndex < editTexts.size -> editTexts[focusIndex].requestFocus()
            else -> {
                editTexts.forEach {
                    it.clearFocus()
                }
    ...
    private fun styleEditTexts(){
        for (x in 0 until editTexts.size){
            ...
            when {
                x == focusIndex -> styleHighlighted(et)
                text.isNotEmpty() -> styleFilled(et)
                else -> styleDefault(et)
            }
        }
    }
    ...
    fun showErrorOTP(editText: EditText? = null){
        if (editText != null){
            editText.background = errorBackgroundImage
        }else{
            editTexts.forEach { it.background = errorBackgroundImage  }
        }
    }
    ...
    private fun customBackground() : Drawable {
        ...
        shape.cornerRadius = 24.dpTopx.toFloat()
